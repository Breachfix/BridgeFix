...
func generateMovies(_ count: Int) -> [Movie] {
    ...
    for _ in 0..<count {
        ...
        let movie = Movie(id: id, name: name, thumbnailURL: thumbnail, categories: categories, genre: .AllGenres, year: year, rating: "PG-13", numberOfSeasons: numSeasons, resolution: "4K", currentEpisode: defEpiInfo, defaultEpisodInfo: defEpiInfo, creators: creators, cast: cast, moreLikeThisMovies: moreLikeThis, episodes: allEpisodes, promotionHeadline: headline, trailers: allTrailers, previewImageName: "", previewVideoURL: "xUFIZh4qEqU&t=155s")
...
func generateTrailers(_ count: Int) -> [Trailer] {
    ...
    for _ in 0..<count {
        ...
        let trailer = Trailer(id: id, name: name, videoURL: "", thumbnailImageURL: thumbnail)
...
func generateEpisodes(_ count: Int, forSeason season: Int) -> [Episode] {
    ...
    for num in 0..<count {
        ...
        let episode = Episode(id: id, name: name, season: season, episodeNumber: num + 1, thumbnailImageURLString: thumbnail, description: desc, length: randLength, videoURL: "")
